document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderInventory() {
        if (inventoryData.length === 0) {
            ...
            updateSummary({ totalOnhand: 0, totalPR: 0, totalPO: 0, grandTotal: 0 });
            return;
        }

        const showManual = includeManualToggle.checked;
        const summary = { totalOnhand: 0, totalPR: 0, totalPO: 0, grandTotal: 0 };

        const rowsHtml = inventoryData.map(item => {
            let onhand = parseFloat(item.TOTAL_ONHAND || 0);
            let pr = parseFloat(item.TOTAL_PR || 0);
            let po = parseFloat(item.TOTAL_PO || 0);

            if (showManual) {
                onhand = onhand + (parseFloat(item.MANUAL_RECEIPT_PR || 0)) + (parseFloat(item.MANUAL_RECEIPT_PO || 0)) - (parseFloat(item.MANUAL_ISSUE || 0));
                pr = pr - (parseFloat(item.MANUAL_RECEIPT_PR || 0));
                po = po - (parseFloat(item.MANUAL_RECEIPT_PO || 0));

                // Don't show negative PR/PO
                if (pr < 0) pr = 0;
                if (po < 0) po = 0;
            }
                ...
                <tr>
                    ...
                    <td>
                        <span class="po-link" 
                              data-part-no="${item.PART_NO}" 
                              data-desc="${item.DESCRIPTION || ''}">
                            ${Math.round(po).toLocaleString()}
                        </span>
                    </td>
                    <td><span class="badge ${rowTotal > 0 ? 'success' : 'warning'}">
                        ${rowTotal > 0 ? 'Active' : 'Empty'}
                    </span></td>
                </tr>
            `;
        }).join('');
        ...
        // Add event listeners (re-attached because innerHTML changed)
        attachListeners();
    }
    ...
    function attachListeners() {
        document.querySelectorAll('.on-hand-link').forEach(link => {
            link.addEventListener('click', (e) => {
                const partNo = e.currentTarget.getAttribute('data-part-no');
        ...
        try {
            ...
            if (result.status === 'success') {
                if (result.data.length === 0) {
                    ...
                    // Grouping Logic... (remaining existing code)
                    const groups = {
                        'ค้าง Move': ['3000'],
                    ...
                    const groupTotals = { 'ค้าง Move': 0, 'Store': 0, 'Warehouse': 0, 'Product': 0, 'Other': 0 };
                    ...
                    if (summaryElem) {
                        summaryElem.innerHTML = `<div class="summary-grid">
    ...
    // (Existing showPRDetail, showPODetail, formatDate, updateSummary, modal close logic...)
        ...
        try {
            ...
            if (result.status === 'success') {
                if (result.data.length === 0) {
                    ...
                    `).join('');
                    prTableBody.innerHTML += `<tr style="background-color: rgba(0,0,0,0.05); font-weight: bold; border-top: 2px solid var(--border-color);"><td colspan="3" style="text-align:right;">ผลรวม:</td><td style="text-align:right; color:#2563eb; font-size: 1.1em;">${Math.round(total).toLocaleString()}</td></tr>`;
                }
            }
        } catch (error) { prTableBody.innerHTML = '<tr><td colspan="4" style="text-align:center; color:red;">Error</td></tr>'; }
    }
    ...
    async function showPODetail(partNo, desc) {
        ...
        try {
            ...
            const result = await response.json();
            if (result.status === 'success') {
                if (result.data.length === 0) { poTableBody.innerHTML = '<tr><td colspan="5" style="text-align:center;">ไม่พบ PO</td></tr>'; }
                else {
                    ...
                    `).join('');
                    poTableBody.innerHTML += `<tr style="background-color: rgba(0,0,0,0.05); font-weight: bold; border-top: 2px solid var(--border-color);"><td colspan="3" style="text-align:right;">ผลรวม:</td><td style="text-align:right;">${Math.round(totalDue).toLocaleString()}</td><td style="text-align:right; color:var(--accent-color); font-size: 1.1em;">${Math.round(totalBalance).toLocaleString()}</td></tr>`;
                }
            }
        } catch (error) { poTableBody.innerHTML = '<tr><td colspan="5" style="text-align:center;">Error</td></tr>'; }
    }
    ...
    function updateSummary(summary) {
        if (!summary) return;
        if (totalOnhandElem) totalOnhandElem.textContent = Math.round(summary.totalOnhand).toLocaleString();
        if (totalPRElem) totalPRElem.textContent = Math.round(summary.totalPR).toLocaleString();
        if (totalPOElem) totalPOElem.textContent = Math.round(summary.totalPO).toLocaleString();
        if (grandTotalElem) grandTotalElem.textContent = Math.round(summary.grandTotal).toLocaleString();
    }

    // Modal Close
    document.querySelectorAll('.close-btn').forEach(btn => {
        btn.onclick = () => { document.getElementById(btn.getAttribute('data-modal')).style.display = 'none'; };
    });
    window.onclick = (e) => { if (e.target.classList.contains('modal')) e.target.style.display = 'none'; };

    // Toggle Change
    includeManualToggle.addEventListener('change', () => {
        renderInventory();
    });

    // filter change
    inventoryFilter.addEventListener('change', (e) => {
        const selectedText = e.target.options[e.target.selectedIndex].text.replace('⭐ ', '').replace(/^\d+\.\)\s*/, '');
    ...
    if (searchInput) {
        searchInput.addEventListener('input', (e) => {
            ...
            rows.forEach(row => {
                ...
                row.style.display = (partNo.includes(term) || partDesc.includes(term)) ? '' : 'none';
            });
        });
    }
